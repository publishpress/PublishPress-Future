<?php

namespace Tests\Modules\Workflows\Domain\Engine\NodeRunners\Actions;

use PublishPress\Future\Core\HookableInterface;
use PublishPress\FuturePro\Modules\Workflows\Domain\Engine\NodeRunners\Actions\CorePostChangeWorkflowStatus;
use PublishPress\FuturePro\Modules\Workflows\Domain\Engine\VariableResolvers\PostResolver;
use PublishPress\FuturePro\Modules\Workflows\Domain\Engine\VariableResolvers\WorkflowResolver;
use PublishPress\FuturePro\Modules\Workflows\Interfaces\NodeRunnerProcessorInterface;
use PublishPress\FuturePro\Modules\Workflows\Models\PostModel;


class CorePostChangeWorkflowStatusTest extends \lucatume\WPBrowser\TestCase\WPTestCase
{
    /**
     * @var \IntegrationTester
     */
    protected $tester;

    public function setUp() :void
    {
        // Before...
        parent::setUp();

        // Your set up methods here.
    }

    public function tearDown() :void
    {
        // Your tear down methods here.

        // Then...
        parent::tearDown();
    }

    private function createWorkflows(): array
    {
        $ids = [];

        $ids[] = wp_insert_post([
            'post_title' => 'Test workflow 1',
            'post_content' => 'Test content',
            'post_status' => 'publish',
            'post_type' => 'workflow'
        ]);

        $ids[] = wp_insert_post([
            'post_title' => 'Test workflow 2',
            'post_content' => 'Test content',
            'post_status' => 'publish',
            'post_type' => 'workflow'
        ]);

        return $ids;
    }

    public function testDisablingWorkflowOnPost(): void
    {
        $postId = wp_insert_post([
            'post_title' => 'Test post',
            'post_content' => 'Test content',
            'post_status' => 'publish',
            'post_type' => 'post'
        ]);

        $workflows = $this->createWorkflows();

        $runner = new CorePostChangeWorkflowStatus(
            $this->makeEmpty(HookableInterface::class),
            $this->makeEmpty(
                NodeRunnerProcessorInterface::class,
                [
                    'getVariableValueFromContextVariables' => new WorkflowResolver(['id' => $workflows[0]])
                ]
            )
        );

        $model = new PostModel();
        $model->load($postId);

        $model->addManuallyEnabledWorkflow($workflows[0]);

        $this->assertEquals([$workflows[0]], $model->getManuallyEnabledWorkflows());

        $runner->actionCallback(
            $postId,
            [
                'post' => [
                    'variable' => 'onManualEnableForPost1.post',
                ],
                'workflow' => [
                    'variable' => 'global.workflow',
                ],
                'status' => [
                    'variable' => 'disable',
                ],
            ],
            [],
            [
                'global.workflow' => new WorkflowResolver(['id' => $workflows[0]]),
            ]
        );

        $this->assertEmpty($model->getManuallyEnabledWorkflows());
    }

    public function testEnablingWorkflowOnPost(): void
    {
        $postId = wp_insert_post([
            'post_title' => 'Test post',
            'post_content' => 'Test content',
            'post_status' => 'publish',
            'post_type' => 'post'
        ]);

        $workflows = $this->createWorkflows();

        $runner = new CorePostChangeWorkflowStatus(
            $this->makeEmpty(HookableInterface::class),
            $this->makeEmpty(
                NodeRunnerProcessorInterface::class,
                [
                    'getVariableValueFromContextVariables' => new WorkflowResolver(['id' => $workflows[0]])
                ]
            )
        );

        $model = new PostModel();
        $model->load($postId);

        $this->assertEmpty($model->getManuallyEnabledWorkflows());

        $runner->actionCallback(
            $postId,
            [
                'post' => [
                    'variable' => 'onManualEnableForPost1.post',
                ],
                'workflow' => [
                    'variable' => 'global.workflow',
                ],
                'status' => [
                    'variable' => 'enable',
                ],
            ],
            [],
            [
                'global.workflow' => new WorkflowResolver(['id' => $workflows[0]]),
            ]
        );

        $this->assertEquals([$workflows[0]], $model->getManuallyEnabledWorkflows());
    }

    public function testTogglingWorkflowOnPostWithWorkflowEnabled(): void
    {
        $postId = wp_insert_post([
            'post_title' => 'Test post',
            'post_content' => 'Test content',
            'post_status' => 'publish',
            'post_type' => 'post'
        ]);

        $workflows = $this->createWorkflows();

        $runner = new CorePostChangeWorkflowStatus(
            $this->makeEmpty(HookableInterface::class),
            $this->makeEmpty(
                NodeRunnerProcessorInterface::class,
                [
                    'getVariableValueFromContextVariables' => new WorkflowResolver(['id' => $workflows[0]])
                ]
            )
        );

        $model = new PostModel();
        $model->load($postId);

        $model->addManuallyEnabledWorkflow($workflows[0]);

        $this->assertEquals([$workflows[0]], $model->getManuallyEnabledWorkflows());

        $runner->actionCallback(
            $postId,
            [
                'post' => [
                    'variable' => 'onManualEnableForPost1.post',
                ],
                'workflow' => [
                    'variable' => 'global.workflow',
                ],
                'status' => [
                    'variable' => 'toggle',
                ],
            ],
            [],
            [
                'global.workflow' => new WorkflowResolver(['id' => $workflows[0]]),
            ]
        );

        $this->assertEmpty($model->getManuallyEnabledWorkflows());
    }

    public function testTogglingWorkflowOnPostWithWorkflowDisabled(): void
    {
        $postId = wp_insert_post([
            'post_title' => 'Test post',
            'post_content' => 'Test content',
            'post_status' => 'publish',
            'post_type' => 'post'
        ]);

        $workflows = $this->createWorkflows();

        $runner = new CorePostChangeWorkflowStatus(
            $this->makeEmpty(HookableInterface::class),
            $this->makeEmpty(
                NodeRunnerProcessorInterface::class,
                [
                    'getVariableValueFromContextVariables' => new WorkflowResolver(['id' => $workflows[0]])
                ]
            )
        );

        $model = new PostModel();
        $model->load($postId);

        $this->assertEmpty($model->getManuallyEnabledWorkflows());

        $runner->actionCallback(
            $postId,
            [
                'post' => [
                    'variable' => 'onManualEnableForPost1.post',
                ],
                'workflow' => [
                    'variable' => 'global.workflow',
                ],
                'status' => [
                    'variable' => 'toggle',
                ],
            ],
            [],
            [
                'global.workflow' => new WorkflowResolver(['id' => $workflows[0]]),
            ]
        );

        $this->assertEquals([$workflows[0]], $model->getManuallyEnabledWorkflows());
    }
}
