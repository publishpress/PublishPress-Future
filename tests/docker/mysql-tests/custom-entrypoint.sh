...
_main() {
	...
	if [ "$1" = 'mysqld' ] && ! _mysql_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			...
			mkdir -p /tmp/docker-entrypoint-initdb.tmp.d/

			# check dir permissions to reduce likelihood of half-initialized database
			ls /docker-entrypoint-initdb.d/ > /dev/null
			ls /tmp/docker-entrypoint-initdb.tmp.d/ > /dev/null
			...
			# Copy the DB files to not override them when replacing
			cp /docker-entrypoint-initdb.d/* /tmp/docker-entrypoint-initdb.tmp.d/

            if [[ -n "${NEW_WP_DOMAIN}" ]]; then
                if [[ -z "${DEPRECATED_WP_DOMAIN}" ]]; then
                    DEPRECATED_WP_DOMAIN=tests.local
                fi

                # Replaces the default test domain on scripts before running them into the DB
                find /tmp/docker-entrypoint-initdb.tmp.d/ -type f -exec sed -i "s/$DEPRECATED_WP_DOMAIN/$NEW_WP_DOMAIN/g" {} \;
            fi


			docker_process_init_files /tmp/docker-entrypoint-initdb.tmp.d/*
