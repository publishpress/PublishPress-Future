...
export function DateOffset({ name, label, defaultValue, onChange, variables = [] }) {
    ...
    return (
        <>
            <VStack>
                <ToggleControl
                    label={__("Avoid duplicated schedule", "publishpress-future-pro")}
                    checked={defaultValue?.avoidDuplicated || false}
                    onChange={(value) => onChangeSetting({ settingName: "avoidDuplicated", value })}
                />
